...
function normalizeIntake(page) {
  ...
  return {
    ...
    format: readProp(page, ip.format),          // Short / Long / Carousel — for the long/short split
...
// Find a board's real status/select option whose name matches `match` (a regex
// or a leading string), so status writes survive per-board naming differences —
// e.g. Chris "5- Script Approval Brand/Creator" vs Brad "5- Script Approval -
// Brand/Creator". Returns the exact option name, or null if none matches.
async function findStatusOption(databaseId, propName, match) {
  if (!databaseId || !propName) return null;
  try {
    const db = await notion.databases.retrieve({ database_id: databaseId });
    const p = db.properties[propName];
    if (!p) return null;
    const bag = p.status || p.select;
    const opts = bag && bag.options ? bag.options : [];
    for (const o of opts) {
      const name = o.name || "";
      if (match instanceof RegExp ? match.test(name) : name.trim().startsWith(match)) return name;
    }
  } catch (e) {}
  return null;
}

module.exports = { notion, fetchAllTasks, fetchIntakeTasks, invalidate, canonEditor, findStatusOption };
